...
# Define the function
def BS_EuCall_MC_IS(S0: float, r: float, sigma: float, K: float, T: int, mu: float, N: int, alpha: float, seed: int, set_seed: bool) -> tuple:
    ...
    for var in [S0, r, sigma, K, mu, alpha]:
...
# Test the function
BS_EuCall_MC_IS(S0, r, sigma, K, T, mu, N, alpha, seed, set_seed)
...
for ind, mu_ in enumerate(mu):
    V0_hat[ind] = BS_EuCall_MC_IS(S0, r, sigma, K, T, mu_, N, alpha, seed, set_seed)[0]
