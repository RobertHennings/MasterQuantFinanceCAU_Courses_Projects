...
```{r}
setwd("X:/Doctor/Stat1/tutorials 2022-2023/PC Tutorials") # your path here!
```
...
```{r   warning=FALSE, error=TRUE}
vec2 = seq(from = 3, to = 15, by = 2)
vec2
View(as.matrix(vec2))
```
```{r}
x = 5*3
vec2 = seq(from = 3, to = x, by = 2)
vec2
```
...
s2 = seq(from = 1, by = 1, length.out = 10)
s3 = seq(from = 1, length.out = 10, by = 1)
# Use the names of the arguments to make your code easily readable.  
s4 = seq(1, 10, 1)
s
s2
s3
...
```{r}
# Another syntax to create a sequence of integers starting at 1 ending at 10
s4 = 1:10
s4
```
...
```
```{r}
mysecondfunction = function(y){
  return(y)
}

mysecondfunction('Hello World')
mysecondfunction(7)
```

```{r}
mythirdfunction = function(y = "Hello World"){
  return(y)
}
...
```{r}
myfourthfunction = function(x,y){
  return(c(x,y))
}

myfourthfunction('Hello', 'World')
```
```{r}
myfourthfunction = function(x,y){
