...
# C-Exercise 09
# Generate samples from the beta distribution by the acceptance/rejection method
...
# that is approximately the mode of the distribution
...
def Sample_dist_AR(N: int, alpha: float, beta: float, c: float, seed: int, set_seed: bool) -> np.array:
    ...
    while counter < N:
        ...
        c_scaler = c
        if U1 <= (1/c_scaler) * f_beta(alpha=alpha, beta=beta, x=U2):
            result[counter] = U2
            counter += 1
    return result

# Next generate N = 10000 samples and compare them in a historgram to the real Beta distribution
sample = Sample_dist_AR(N=10000, alpha=2.0, beta=5.0, c=c, seed=234, set_seed=True)
